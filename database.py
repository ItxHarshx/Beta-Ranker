...
async def create_user_if_not_exists(user_id):
    ...
    await conn.execute(
        """INSERT INTO users (id, health, gold_coins, exp, level, essence)
           VALUES ($1, 100, 0, 0, 1, 0) ON CONFLICT (id) DO NOTHING""",
        user_id
    )
    await conn.close()
...
async def get_user_data(user_id):
    conn = await connect_db()
    user = await conn.fetchrow("SELECT health, gold_coins, exp, level, essence FROM users WHERE id = $1", user_id)
    await conn.close()
